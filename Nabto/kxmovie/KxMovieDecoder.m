...
#ifdef DEBUG
static void __unused fillSignal(SInt16 *outData,  UInt32 numFrames, UInt32 numChannels)
{
...
static void __unused fillSignalF(float *outData,  UInt32 numFrames, UInt32 numChannels)
{
    static float phase = 0.0;
...
static void __unused testConvertYUV420pToRGB(AVFrame * frame, uint8_t *outbuf, int linesize, int height)
{
...
{
    if (!path) {
        LoggerVideo(2, @"openFile nil path");
        return NO;
    }
    if (_formatCtx) {
        LoggerVideo(2, @"openFile context already open");
        return NO;
    }
...
- (kxMovieError) openInput: (NSString *) path
{
    ...
    // Force TCP connection to avoid wasting 10 seconds trying to stream through UDT
    // https://github.com/kolyvan/kxmovie/issues/21
    ...
    errCode = avformat_open_input(&formatCtx, [path cStringUsingEncoding: NSUTF8StringEncoding], NULL, &options);
    if (errCode < 0) {
        LoggerVideo(1, @"avformat_open_input error: %s", av_err2str(errCode));
        if (formatCtx)
...
{    
    ...
    LoggerVideo(1, @"video codec size: %u:%u fps: %.3f tb: %f",
                (unsigned int)self.frameWidth,
                (unsigned int)self.frameHeight,
                _fps,
                _videoTimeBase);
...
- (kxMovieError) openSubtitleStream: (NSInteger) subtitleStream
{
    ...
    if (codecDesc && codecDesc->name) {
        ...
        LoggerStream(1, @"subtitle codec: 'NO NAME' mode: %d enc: %s",
                     codecCtx->sub_charenc_mode,
                     codecCtx->sub_charenc);
    }
...
- (KxAudioFrame *) handleAudioFrame
{
    ...
    if (_swrContext) {
        ...
        const unsigned int ratio = MAX(1, audioManager.samplingRate / _audioCodecCtx->sample_rate) *
...
static int interrupt_callback(void *ctx)
{
    ...
    if (r) LoggerStream(1, @"KxMovieDecoder: interrupt callback!");
    return r;
}
...
static void FFLog(void* context, int level, const char* format, va_list args) {
    @autoreleasepool {
        ...
        // TODO: Get 401 error from avformat_open_input
        if ([message rangeOfString:@"401 Unauthorized"].location != NSNotFound) {
            LoggerVideo(1, @"401 Authentication error");
            [[NSNotificationCenter defaultCenter] postNotificationName:@"decoderAuthenticationError" object:nil];
        }
